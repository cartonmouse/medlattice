...
本实验服务于工程学习和面试复盘，不代表医疗能力、临床安全性或人类偏好对齐效果。当前没有可合法使用的人类/临床专家偏好数据，因此采用可审计的合成偏好对；这个限制必须在面试中主动说明。
...
SFT 让模型模仿 chosen answer，DPO 则在同一个 prompt 下显式拉开 chosen 与 rejected 的相对偏好。DPO 不需要单独训练 reward model，也不需要在线 PPO，但它仍然依赖高质量、合法授权的偏好对。当前项目先用合成对验证链路，后续要换成人类或领域专家偏好。

### 你的 chosen/rejected 是怎么来的？

chosen 是 CMB 参考答案字母，rejected 是其他选项中固定随机种子抽到的错误字母。因为 rejected 很容易构造，所以它不是人类偏好标注；它可能只让模型更会区分答案字母，而没有学到开放式医疗回答的质量、安全和解释能力。

### DPO 最后有效果吗？

训练目标有效优化，最终 reward margin 为 0.1947；但独立 240 条 val 上 SFT 和 DPO 都是 121/240、50.42%，只有 8 条预测改变，净提升为 0。这是一个诚实的负结果：DPO 工程做成了，但当前数据和任务设计没有证明下游能力提升。

### 下一步怎样让 DPO 更有说服力？

需要合法授权的开放式医疗问答偏好对，至少包含事实性、引用完整性、拒答边界和表达质量等维度，并由医生或经过训练的标注者审核；同时要保留独立 test/val、做多随机种子和人工评测，避免把“容易的错误选项负例”包装成真实对齐结果。
