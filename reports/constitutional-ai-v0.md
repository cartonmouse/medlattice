...
因此，本阶段应作为“安全边界和审计链路”的工程基线，而不是简历中夸大的医疗能力指标。
...
> 我没有把 Constitutional AI 简化成一句安全 prompt，而是先把医疗场景的安全要求版本化为五条原则，并实现了候选回答的批评、固定模板修订和再批评闭环。12 条合成样本中 8 条初始违规，8 条都被修订到规则通过。这个 v0 的价值是可审计和可回归，不是证明医疗安全达到 100%；下一步会用合法脱敏、领域审核的数据比较规则 critic、模型 critic 和人工复核。
