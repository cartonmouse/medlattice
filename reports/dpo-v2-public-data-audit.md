...
数据只用于机器学习工程学习和面试复盘，不代表医疗能力、临床安全性或医生认可。公开数据的 `chosen` 不应默认解释为专家结论，尤其不能把自动或模型辅助的偏好判断包装成人类标注。
...
若继续改进，应优先选择中文、开放式、经人工/专家审核的偏好对，减少语言迁移；再增加训练样本和多 seed，并补充人工 pairwise 评审。当前结果不建议把 public DPO adapter 作为简历上的主模型，简历主结果应继续使用已有 QLoRA/SFT 与 RAG 对照，DPO 作为有明确失败分析的消融。

## 8. 面试解释

### 为什么不直接下载完整 train？

本轮目标是先验证数据结构、许可证记录、偏好方向、去重和独立评测隔离。完整 train 体积大，而且对当前 8GB GPU 的第一轮实验没有必要；固定 revision 的 dev/test 已经可以形成可审计小实验。

### 为什么把 human 样本放在 test？

它们只作为外部 preference holdout，不参与训练和内部切分。这样可以区分“模型在训练同分布 pair 上下降 loss”和“模型是否更匹配未参与训练的人工偏好”。

### 这个数据能证明医疗能力吗？

不能。它是公开英文偏好数据，偏好来源包含自动/模型辅助判断的可能，且本项目主任务是中文 CMB 选择题。最多能证明 DPO 数据接入和偏好 proxy 的实验链路，不能声称临床对齐或医疗准确率。
